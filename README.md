...
1. `overdue_v2_0_1.html`（または `index.html`）をダウンロードする
2. ブラウザでファイルを開く
...
- アプリケーション: **v2.0.1**（HTML 内の `const APP_VER` が唯一の版数の出所）
...
| 版 | 主な変更 |
|---|---|
| **v2.0.1** | **脆弱性係数のレンジガードを是正。** 従来は取込・入力の4経路（手入力／MAAZ の JSON 取込／保存済み JSON の読み込み／ヒアリングシート取込）のうち、レンジガード（[0,1] へのクランプ）が効いていたのはヒアリングシート経路だけだった。残る3経路は範囲外の値（例: `vulnCoeff=2.0`）がそのまま台帳に反映されてしまう穴があったため、同一のガードを4経路すべてに適用し、範囲外の値はクランプまたは取込拒否のうえ画面に通知するようにした（保存済み JSON の読み込みのみ下限は 0 のまま — 一度保存した帳簿を読み戻せなくなる方が実害が大きいため）。あわせて、MAAZ の成熟度データから自動算出する脆弱性係数に **下限 0.03** を新設した（MAAZ 側の自前計算とそろえる形。SCF＝補正の効き具合を表す値には床を設けていない）。**この結果、脅威技法を全カバーした場合の DLP 側侵害確率（`dlpProb_asis`）が 3.00% → 3.45% に変わる**（意図した挙動変更。「対策を尽くしてもゼロにはならない」という本ツールの主張に整合させたもの）。また、壊れた MAAZ の JSON（キー欠落・`total=0`・異常に大きい値など）を取り込んだ際に不正な数値が台帳へ混入していた不具合を、取込全体を拒否して既存の帳簿を一切変更しない方式に是正した（拒否時はその旨と「既存の帳簿は変更していません」を画面に表示）。**計算エンジン・恒等式・エクスポート形式（`format` / `engine` キー）に変更はない**（上記の下限 0.03 導入による出力差を除く。v2.0.0 以前の保存 JSON はそのまま読み込めます） |
| **v2.0.0** | v1.10.0 に続く**ダッシュボード・ファースト**の仕上げ。(a) 01 内訳台帳の章冒頭に**4分類オーバービュー**を追加 — 章を読む前に4分類の残高感を掴めるカードから、該当する明細行へスクロールジャンプできる（表示のみ・状態は変えない）。(b) 02 猶予時間を**2カラム化**（左＝猶予カーブ、右＝操作パネル）。あわせて SVG が狭い描画幅でも読めるよう、各行の出典テキストをグラフ外の一覧へ退避し、描画倍率が下がる分 SVG 内フォントを 11/12/13px → 15/16/17px に引き上げ（Ztelier 下限 11px は screen 実測で確保）。(c) 04 計上・入力（データの入れ方）を **STEP 1〜3** 構成に再編（STEP 1 取り込む → STEP 2 手で入れる → STEP 3 保存して出す）。(d) v1.10.0 で章ヘッダ直下の折りたたみへ退避していたヒーロー直下の**日本語リード文（lede）を常時表示に復帰**（「ヒーロー文の下の折り畳みは不毛」というユーザー裁定 2026-08-12 による。文言は一字も変えていない）。あわせて、折りたたみ移設時に縮小されていた**ヒーロー見出し（compact 表示時）のフォントサイズを回復**（19–28px → 34–52px、モバイル 20px → 26px）。lede 以外の折りたたみ（読み方カード・05 根拠開示の各解説等）は従来どおり維持。**計算エンジン・恒等式・定数・エクスポート形式（`format` / `engine` キー）に変更はない**（v1.10.0 以前の保存 JSON はそのまま読み込めます） |
...
| **v1.1.1** | 初回公開（v1.2〜v1.6 は内部検証のみで、公開版はありません） |
